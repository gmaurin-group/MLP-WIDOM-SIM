...
# this _vdw_overlap used in the widom code
def _vdw_overlap(atoms, vdw, n_ads, n_frame):
...
def check_geometric_overlap(atoms, n_frame, n_ads, select_molecule, cutoff_distance):
    nat = len(atoms)
    indices_select_molecule = range(n_frame + n_ads * select_molecule, n_frame + n_ads * (select_molecule + 1))
    indices_surrounding_system = [i for i in range(nat) if i not in indices_select_molecule]
    distances_select_to_surrounding = [atoms.get_distances(i, indices_surrounding_system, mic=True) for i in indices_select_molecule]
    return np.min(distances_select_to_surrounding) < cutoff_distance  
...
class PREOS(EOS):
    ...
    def from_name(cls, compound):
        ...
        fn = os.path.join(src_dir, 'critical_acentric.csv')
